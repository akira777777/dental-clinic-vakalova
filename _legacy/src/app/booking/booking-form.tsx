...
export default function BookingForm({ services, doctors }: BookingFormProps) {
  ...
  useEffect(() => {
    ...
    const available = doctors.filter(doc =>
      doc.services.some(s => s.id === bookingData.serviceId)
    );

    const isCurrentAvailable = available.some(d => d.id === bookingData.doctorId);

    if (!isCurrentAvailable && available.length > 0) {
       setBookingData(prev => ({ ...prev, doctorId: available[0].id }));
    } else if (available.length === 0) {
       setBookingData(prev => ({ ...prev, doctorId: "" }));
    }
  }, [bookingData.serviceId, doctors]); // Removed bookingData.doctorId to prevent loop
  ...
  const handleDateClick = (date: Date) => {
    // Store as ISO string date part only to avoid timezone issues or just ISO
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-2xl text-center bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-12 shadow-card">
              ...
              <p className="mb-2 text-lg text-zinc-700 dark:text-zinc-300">
                Ваша запись на <span className="font-semibold">{bookingData.date ? format(new Date(bookingData.date), 'd MMMM yyyy', { locale: ru }) : ''}</span> в{" "}
      ...
      <main className="flex-grow bg-white dark:bg-zinc-950 pt-20 pb-28 lg:pb-12">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-sm font-semibold text-zinc-900 dark:text-white">
                  {selectedService?.name || "Выберите услугу"}
                </p>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                   {bookingData.date ? format(new Date(bookingData.date), 'd MMM', { locale: ru }) : "Дата не выбрана"} • {bookingData.time}
                </p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-primary">{selectedService?.price} Kč</p>
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-96 overflow-y-auto pr-2">
                  {filteredServices.map((service) => (
                    <label
                      key={service.id}
                      className={`relative flex items-start p-4 cursor-pointer rounded-lg border transition-all ${bookingData.serviceId === service.id
                    ...
                    >
                      <input
                        ...
                        onChange={() => setBookingData({ ...bookingData, serviceId: service.id })}
                ...
                {bookingData.serviceId && (
                  <div className="mt-6">
                    <h3 className="text-sm font-medium text-zinc-900 dark:text-white mb-3">Выберите врача</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {availableDoctors.map((doc) => (
                        <label
                          key={doc.id}
                          className={`relative flex items-center p-3 cursor-pointer rounded-lg border transition-all ${bookingData.doctorId === doc.id
                              ? "border-primary bg-blue-50/50 dark:bg-blue-900/20 dark:border-primary"
                              : "border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-600"
                            }`}
                        >
                           <input
                            type="radio"
                            name="doctor"
                            checked={bookingData.doctorId === doc.id}
                            onChange={() => setBookingData({ ...bookingData, doctorId: doc.id })}
                            className="mr-3 h-4 w-4 text-primary border-zinc-300 focus:ring-primary"
                          />
                          <div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-7 gap-1 text-center">
                      {calendarDays.map((day, i) => {
                        const isSelected = bookingData.date && isSameDay(day, new Date(bookingData.date));
                        ...
                        return (
                          <button
                            ...
                            className={`p-2 text-sm rounded-lg transition-colors
                              ${!isCurrentMonth ? "text-zinc-300 dark:text-zinc-700" : "text-zinc-900 dark:text-white"}
                              ${isSelected ? "bg-primary text-white shadow-sm" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-lg overflow-hidden">
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div>
                        <p className="text-sm font-medium text-zinc-900 dark:text-white">
                           {bookingData.date ? format(new Date(bookingData.date), 'd MMMM', { locale: ru }) : "Дата не выбрана"}
                        </p>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-zinc-900 dark:text-white font-medium">{selectedService?.price || 0} Kč</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between text-base font-semibold pt-2">
                        ...
                        <span className="text-primary">{selectedService?.price || 0} Kč</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-6 pt-0 hidden lg:block">
                    <button
                      ...
                      disabled={isSubmitting || !bookingData.firstName || !bookingData.lastName || !bookingData.phone || !bookingData.email || !bookingData.serviceId || !bookingData.doctorId || !bookingData.date}
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 p-4 shadow-2xl z-40">
            ...
            >
              <CheckCircle className="h-5 w-5" />
              Подтвердить запись за {selectedService?.price || 0} Kč
            </button>
          </div>
        </div>
      </main>
